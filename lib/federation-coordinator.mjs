 ...
 *  - `GET /roster` (owner decision, 2026-07-21): a READ-ONLY directory of the
 *    roster's PUBLIC keys — `[{key_id, public_key_pem}]` — gated by the same
 *    collective bearer token as every other endpoint. An earlier revision of
 *    this file had no such endpoint and a caller-side comment described that
 *    absence as deliberate; it was a gap, not a security choice — public
 *    keys are public by definition, so serving them to anyone already inside
 *    the collective token gate discloses nothing new and does not weaken the
 *    trust model. This is the mechanism that lets `waspflow federation join`/
 *    `contribute`/`submit` auto-populate a member's local roster cache
 *    instead of requiring every public key to be hand-relayed and pasted via
 *    `waspflow federation trust`. It is READ-ONLY: there is deliberately no
 *    corresponding `POST /roster` or any other way to register/add a key
 *    over the network. `deps.roster` itself remains the hand-edited,
 *    hot-reloaded local file — MEMBERSHIP (who gets added to that file at
 *    all) stays a real human decision the coordinator operator makes out of
 *    band; this endpoint only removes the redundant human relay of already-
 *    approved members' already-public keys.
 */
...
function publicView(record) {
  const task = record.task_envelope?.payload;
  return {
...
function requireToken(request, token) {
  ...
  if (!match || match[1] !== activeToken) fail('missing or invalid collective bearer token', 401);
}
...
// The authenticated task-discovery projection used by both GET /tasks and
// GET /tasks/next. It deliberately returns only records that can be claimed
// *now*: QUEUED, plus CLAIMED records that this read lazily requeues because
// their lease has expired. The full signed envelope is still fetched and
// verified by the normal claim/pull path; this is discovery metadata only.
//
// v0 deliberately scans every record and has no pagination. That is suitable
// for the friends-and-family scale this coordinator targets, but a larger
// collective should add a queryable index and a bounded page contract.
async function claimableTasks(deps) {
  let entries;
  try {
...
function discoveryView(record) {
  const payload = record.task_envelope.payload;
  return {
    task_digest: record.task_digest,
    display_id: payload.display_id,
    // The coordinator record already has the signed author key. Discovery is
    // authenticated, so expose that useful attribution rather than making the
    // contributor review a misleading "Not reported" placeholder.
    author: payload.author_key,
    published_at: record.published_at,
    // These are the resource-relevant signed fields task v0 currently
    // carries. There is no harness or human-readable description field in
    // the v0 schema; clients can use these artifact/network hints to filter
    // without treating the discovery response itself as trusted execution
    // input.
    network: payload.network,
    source: payload.source,
    ...(payload.git_source ? { git_source: payload.git_source } : {}),
    prompt: payload.prompt,
  };
}

// GET /tasks — return every currently claimable task so a contributor can
// choose. Authenticated (unlike GET /tasks/:digest): this is a discovery
// surface across all tasks, so it uses the same collective bearer-token gate
// as claim/submit/publish.
async function handleList({ request, respond, deps }) {
  requireToken(request, deps.token);
  respond(200, (await claimableTasks(deps)).map(discoveryView));
}

// GET /requests?author=<key-id> — the requester's complete own history. This
// is separate from claimable discovery because settled and currently claimed
// tasks are essential request history but must not be offered to contributors.
async function handleRequests({ request, respond, deps }) {
  requireToken(request, deps.token);
  const author = new URL(request.url, 'http://coordinator.internal').searchParams.get('author');
  if (!author) fail('author query parameter is required', 400);
  let entries;
  try { entries = await readdir(deps.dataDir, { withFileTypes: true }); } catch (error) { if (error.code === 'ENOENT') return respond(200, []); throw error; }
  const now = Date.now();
  const requests = [];
  for (const entry of entries) {
    ...
    if (view.author === author) requests.push({ ...view, has_result: Boolean(view.result_envelope) });
  }
  requests.sort((a, b) => String(b.published_at || '').localeCompare(String(a.published_at || '')));
  respond(200, requests);
}

// Collective activity is intentionally task-level only. It exposes the
// shared lifecycle/attribution already stored by the coordinator, never an
// executor's local receipt, transcript, account identity, or tool output.
async function handleActivity({ request, respond, deps }) {
  requireToken(request, deps.token);
  let entries;
  try { entries = await readdir(deps.dataDir, { withFileTypes: true }); } catch (error) { if (error.code === 'ENOENT') return respond(200, []); throw error; }
  const now = Date.now();
  ...
  for (const entry of entries) {
    ...
    const task = record.task_envelope?.payload || {};
    activity.push({
      task_digest: `sha256:${record.task_digest}`,
      display_id: task.display_id || null,
      author: task.author_key || null,
      executor_key: record.executor_key || null,
  ...
  activity.sort((left, right) => String(right.settled_at || right.claimed_at || right.published_at || '').localeCompare(String(left.settled_at || left.claimed_at || left.published_at || '')));
  respond(200, activity);
}

// GET /tasks/next — retained as the one-click, "just give me one" convenience
// for the guided contributor flow. It uses the exact discovery scan above so
// its claimability and lazy-expiry behavior cannot diverge from GET /tasks.
async function handleNext({ request, respond, deps }) {
  requireToken(request, deps.token);
  const [next] = await claimableTasks(deps);
  respond(200, { task_digest: next ? next.task_digest : null });
}

// GET /roster — read-only public-key directory (owner decision, 2026-07-21).
// See this module's top-of-file doc for why this is safe to serve (public
// keys are public) and why it is deliberately READ-ONLY (no POST/register
// counterpart exists or should exist — membership stays a human decision
// enacted by editing the hand-edited roster file, not a network write).
// Gated by the same collective bearer token as every other endpoint (this
// is a discovery surface across the whole roster, not a single-digest
// unguessable-token lookup like GET /tasks/:digest, so it needs the token
// gate the same way GET /tasks/next does).
async function handleRoster({ request, respond, deps }) {
  requireToken(request, deps.token);
  const entries = Array.from(deps.roster.entries()).map(([keyId, publicKeyPem]) => ({ key_id: keyId, public_key_pem: publicKeyPem }));
  respond(200, { roster: entries, ...(deps.collectiveName ? { collective_name: deps.collectiveName } : {}) });
}
...
function isApiPath(request) {
  const pathname = new URL(request.url, 'http://coordinator.internal').pathname;
  return /^\/(?:tasks|requests|activity|roster|artifacts)(?:\/|$)/.test(pathname);
}
...
<title>Waspflow Federation</title><body><main><h1>Waspflow Federation collective coordinator</h1><p>This coordinator is online.</p><p>Members should open their invite in the Waspflow Federation app or run the join command.</p></main></body></html>`;

const JOIN_PAGE = `<!doctype html>
<html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>Join Waspflow Federation</title><style>body{font:16px system-ui,sans-serif;max-width:42rem;margin:4rem auto;padding:0 1.25rem;color:#17212b;line-height:1.5}h1{line-height:1.2}ol{padding-left:1.5rem}li{margin:.9rem 0}button{font:inherit;padding:.65rem .9rem;border:0;border-radius:.45rem;background:#19597e;color:#fff;cursor:pointer}code{display:block;overflow-wrap:anywhere;padding:.65rem;background:#f1f4f6}#status{min-height:1.5rem;color:#176a46}</style><body><main><h1>You’re invited to Waspflow Federation</h1><ol><li><a href="https://github.com/tnunamak/waspflow/blob/waspflow/fedgui-e2e/README.md#federation">Install Waspflow Federation</a></li><li>Open it</li><li>Paste this invite link</li></ol><button type="button" id="copy-invite">Copy invite link</button><p id="status" aria-live="polite"></p><details><summary>Using a terminal instead?</summary><code id="terminal-command"></code></details></main><script>
const button = document.getElementById('copy-invite');
const status = document.getElementById('status');
document.getElementById('terminal-command').textContent = 'waspflow federation join "' + location.href + '"';
button.addEventListener('click', async () => {
  const invite = location.href;
  try { await navigator.clipboard.writeText(invite); status.textContent = 'Invite link copied.'; }
  catch { status.textContent = invite; }
});
</script></body></html>`;
...
function route(request) {
  ...
  if (request.method === 'GET' && parts.length === 1 && parts[0] === 'tasks') return { name: 'list' };
  if (request.method === 'GET' && parts.length === 1 && parts[0] === 'requests') return { name: 'requests' };
  if (request.method === 'GET' && parts.length === 1 && parts[0] === 'activity') return { name: 'activity' };
  if (request.method === 'GET' && parts.length === 2 && parts[0] === 'tasks' && parts[1] === 'next') return { name: 'next' };
  if (request.method === 'GET' && parts.length === 1 && parts[0] === 'roster') return { name: 'roster' };
...
export function createCoordinatorServer(deps) {
  ...
  if ((typeof deps.token !== 'string' && typeof deps.token !== 'function') || !deps.token) fail('collective token is required', 500);
  ...
  return createServer((request, response) => {
    const respond = (status, body) => {
      ...
      response.writeHead(status, { 'content-type': 'application/json', 'x-waspflow-federation-coordinator-schema': String(COORDINATOR_SCHEMA_VERSION) });
      response.end(json);
    };
      ...
      try {
        ...
        if (!matched) {
          if (wantsBrowserHtml(request) && !isApiPath(request)) {
            const pathname = new URL(request.url, 'http://coordinator.internal').pathname;
            return sendHtml(response, pathname === '/join' ? JOIN_PAGE : LANDING_PAGE);
          }
        ...
        else if (matched.name === 'list') await handleList(ctx);
        else if (matched.name === 'requests') await handleRequests(ctx);
        else if (matched.name === 'activity') await handleActivity(ctx);
        else if (matched.name === 'next') await handleNext(ctx);
        else if (matched.name === 'roster') await handleRoster(ctx);
 ...
 */
export async function startCoordinator({ dataDir, token, roster, collectiveName, port = 0, host = '127.0.0.1' }) {
  await mkdir(dataDir, { recursive: true });
  const server = createCoordinatorServer({ dataDir, token, roster, collectiveName });
