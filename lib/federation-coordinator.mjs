 ...
 *  - Artifact bytes are out of scope: task/result envelopes only carry
 *    digests. Fetching/storing the referenced bytes is a later slice.
 */
import { randomUUID } from 'node:crypto';
import { mkdir, readFile, readdir, rename, writeFile, unlink } from 'node:fs/promises';
import { createServer } from 'node:http';
import { join } from 'node:path';
import { verifyEnvelope, parseCanonicalEnvelope, EnvelopeError } from './federation-envelope.mjs';
    ...
    (async () => {
      try {
        ...
        const body = ['publish', 'claim', 'submit'].includes(matched.name)
          ...
          : Buffer.alloc(0);
        const ctx = { request, respond, body, deps, digest: matched.digest };
