...
<html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>Join Waspflow Federation</title><body><main><h1>You’re invited to a Waspflow Federation collective</h1><p><a href="https://github.com/tnunamak/waspflow#federation">Install Waspflow Federation</a></p><button type="button" id="copy-command">Copy join command</button><p id="status" aria-live="polite"></p></main><script>
const button = document.getElementById('copy-command');
const status = document.getElementById('status');
button.addEventListener('click', async () => {
  const command = 'waspflow federation join "' + location.href + '"';
  try { await navigator.clipboard.writeText(command); status.textContent = 'Join command copied.'; }
