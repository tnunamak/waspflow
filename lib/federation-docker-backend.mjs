...
/** Explicit-only safe mutation used by `waspflow federation doctor --fix-policy`. */
...
export class DockerSbxBackend extends SandboxBackend {
  ...
  async probeCapabilities(options) {
    const preflight = await probeSbxPreflight(options);
    ...
    return { available: true, backend_id: BACKEND_ID, version: preflight.version, preflight };
  }
...
export const _internal = { isSafeRelativeOutputPath, sandboxNameFor, sbxHome, scratchRoot, preflightChecks, packageIsInstalled, containerdIsV2 };
