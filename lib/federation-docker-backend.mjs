...
function preflightChecks({ platformName, version, packages, dockerVersion, containerdVersion, diagnose, policy, kvmReadable, kvmWritable }) {
...
export async function probeSbxPreflight({ runCommand = runPreflightCommand, platformName = process.platform } = {}) {
  ...
  const version = await run(sbxBin(), ['version']);
  const linux = platformName === 'linux';
  const [packages, dockerVersion, containerdVersion, diagnose, policy, kvmReadable, kvmWritable] = await Promise.all([
    ...
    version.errorCode === 'ENOENT' ? Promise.resolve({ code: 1, stderr: 'sbx is not installed' }) : run(sbxBin(), ['diagnose']),
    version.errorCode === 'ENOENT' ? Promise.resolve({ code: 1, stderr: 'sbx is not installed' }) : run(sbxBin(), ['policy', 'ls']),
    linux ? run('test', ['-r', '/dev/kvm']) : Promise.resolve({ code: 0, stdout: '' }),
    linux ? run('test', ['-w', '/dev/kvm']) : Promise.resolve({ code: 0, stdout: '' }),
  ]);
  const checks = preflightChecks({ platformName, version, packages, dockerVersion, containerdVersion, diagnose, policy, kvmReadable, kvmWritable });
  return {
