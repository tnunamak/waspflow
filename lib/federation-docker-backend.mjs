...
export function parseDockerAccount(diagnoseOutput) {
  ...
  const match = /\bSigned in as\s+([^\s(]+)/i.exec(output);
  return match ? match[1] : null;
}

/** Runs `sbx diagnose` under the same isolated, sanitized child env as jobs. */
export async function readDockerAccount() {
  const result = await runSbx(['diagnose']);
  return parseDockerAccount(outputOf(result));
}
