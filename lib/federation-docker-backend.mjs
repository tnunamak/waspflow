 ...
 */
function sbxHome() {
  return process.env.WASPFLOW_FEDERATION_SBX_HOME || path.join(os.homedir(), '.waspflow', 'sbx-home');
}
...
function preflightChecks({ platformName, version, packages, dockerVersion, containerdVersion, diagnose, policy, kvmReadable, kvmWritable, hypervisorPlatform }) {
  if (platformName === 'win32') return windowsPreflightChecks({ version, diagnose, hypervisorPlatform });
    ...
    : transferPluginFailure
      ...
      : !packageIsInstalled(packages, 'docker-ce') || !packageIsInstalled(packages, 'containerd.io')
        ...
        : !commandWorked(dockerVersion) || !containerdIsV2(containerdVersion)
          ? `Docker CE/containerd v2 could not be confirmed: ${outputOf(containerdVersion) || outputOf(dockerVersion) || 'no diagnostic output'}`
          : `Docker CE ${outputOf(dockerVersion)} with ${outputOf(containerdVersion)}`;
  ...
  const daemonDetail = daemonOk
    ...
    : `sbx diagnose did not report a healthy daemon: ${diagnostics || 'no diagnostic output'}`;
  ...
  const policyDetail = policyOk
    ...
    : policyMissing
      ...
      : `sbx policy ls failed: ${policyOutput || 'no diagnostic output'}`;
  ...
  const loginDetail = loginOk
    ...
    : loginMissing
      ? 'sbx reported that the user is not authenticated to Docker.'
      : `Docker login could not be confirmed because sbx diagnose failed: ${diagnostics || 'no diagnostic output'}`;

  return [
    check('sbx_install', sbxInstallOk, sbxInstallDetail, sbxInstallOk ? '' : sbxInstallFix),
    check('docker_runtime', dockerRuntimeOk, dockerRuntimeDetail, dockerRuntimeOk ? '' : UBUNTU_INSTALL_FIX),
    check('sbx_daemon', daemonOk, daemonDetail, daemonOk ? '' : 'sbx daemon stop && sbx daemon start --detach && sbx diagnose'),
    check('network_policy', policyOk, policyDetail, policyOk ? '' : 'sbx policy init balanced (or run: waspflow federation doctor --fix-policy)'),
    check('kvm_access', kvmOk, kvmDetail, kvmOk ? '' : 'sudo usermod -aG kvm $USER && newgrp kvm (on a VM, also enable nested virtualization)'),
    check('docker_login', loginOk, loginDetail, loginOk ? '' : 'sbx login'),
  ];
}
...
function windowsPreflightChecks({ version, diagnose, hypervisorPlatform }) {
  ...
  return [
    check('sbx_install', sbxInstallOk,
      sbxInstallOk ? `sbx found: ${outputOf(version)}` : 'sbx was not found on PATH (checked with `where sbx`).',
      sbxInstallOk ? '' : 'Run the Federation installer\'s Repair action. If repair is unavailable, this device cannot contribute yet.'),
    check('hypervisor_platform', hypervisorEnabled,
      hypervisorEnabled ? 'Windows Hypervisor Platform is enabled.' : `Windows Hypervisor Platform is not enabled: ${outputOf(hypervisorPlatform) || 'no diagnostic output'}`,
      hypervisorEnabled ? '' : 'Reinstall or run the Federation installer\'s Repair action. Last resort for an administrator: Enable-WindowsOptionalFeature -Online -FeatureName HypervisorPlatform -All -NoRestart'),
    check('docker_login', loginOk,
      loginOk ? 'sbx diagnose did not report a Docker authentication problem.' : loginMissing
        ? 'sbx reported that the user is not authenticated to Docker.'
        : `Docker login could not be confirmed because sbx diagnose failed: ${diagnostics || 'no diagnostic output'}`,
      loginOk ? '' : 'Open Federation and select Sign in to Docker. If that action is unavailable, this device cannot contribute yet.'),
    check('windows_file_permissions', true,
      'Windows uses ACLs; POSIX 0600/0700 modes on Federation config files are not enforced here. Installer repair owns any needed permission remediation.',
      ''),
  ];
}
...
function daemonNeedsStart(check) {
  return !check?.ok && /(?:daemon.*(?:connection refused|not running|stopped|unavailable)|connection refused)/i.test(check.detail);
}

function policyNeedsInitialization(check) {
  return !check?.ok && check.detail === 'global network policy has not been initialized.';
}
...
async function ensureSbxIdentity({ runCommand = runPreflightCommand, platformName = process.platform } = {}) {
  ...
  if (daemonNeedsStart(daemon)) {
    ...
    repairs.push({ name: 'sbx_daemon', ok: result.code === 0, detail: outputOf(result) });
  ...
  if (policyNeedsInitialization(policy)) {
    ...
    repairs.push({ name: 'network_policy', ok: result.code === 0, detail: outputOf(result) });
    if (result.code === 0) preflight = await probeSbxPreflight({ runCommand, platformName });
  }
...
export const _internal = { isSafeRelativeOutputPath, sandboxNameFor, sbxHome, scratchRoot, preflightChecks, packageIsInstalled, containerdIsV2, daemonNeedsStart, policyNeedsInitialization };
