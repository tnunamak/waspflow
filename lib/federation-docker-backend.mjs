...
async function sbxAvailable() {
  try {
    await execFile(sbxBin(), ['--version'], { env: sbxChildEnv() });
    return true;
  } catch (error) {
    if (error && error.code === 'ENOENT') return false;
    // Any other failure (e.g. non-zero exit from --version) still means the
    // binary exists; only ENOENT means "not installed".
    return true;
  }
}
...
export class DockerSbxBackend extends SandboxBackend {
  ...
  async probeCapabilities() {
    const available = await sbxAvailable();
    if (!available) {
      return {
        available: false,
        backend_id: BACKEND_ID,
        missing_prerequisites: ['sbx CLI (Docker Sandboxes) not found on PATH'],
        install_hint: INSTALL_HINT,
      };
    }
    let version;
    try {
      const { stdout } = await execFile(sbxBin(), ['--version'], { env: sbxChildEnv() });
      version = stdout.trim();
    } catch {
      version = undefined;
    }
    return { available: true, backend_id: BACKEND_ID, version };
  }
  ...
  async prepare(validatedJob) {
    ...
    const runResult = await runSbx(['run', '--name', sandboxId, scratchDir, validatedJob.image]);
    if (runResult.code !== 0) {
  ...
  /**
   * UNVERIFIED SBX SYNTAX: `sbx cp <name>:<remote-path> <local-path>` (and the
   * reverse direction for copy-out) is a best-guess based on common CLI
   * conventions (cf. `docker cp`), not confirmed against `sbx cp --help`.
   * Needs a follow-up check on a machine with sbx installed.
   */
  async _copyIn(handle, input) {
    ...
    const remoteDest = `${handle.sandbox_id}:${input.dest}`;
  ...
  async start(handle) {
    // UNVERIFIED SBX SYNTAX: `sbx exec <name> -- <cmd...>` is a best-guess
    // invocation shape (cf. `docker exec`), not confirmed against
    // `sbx exec --help`. Needs a follow-up check on a machine with sbx
    // installed; the exact flag for detaching/backgrounding execution is
    // also unconfirmed.
    const result = await runSbx(['exec', handle.sandbox_id, '--', handle._entrypoint]);
    if (result.code !== 0) {
  ...
  async collectDeclaredOutputs(handle, manifest) {
    ...
    for (const declaredPath of manifest || []) {
      ...
      // UNVERIFIED SBX SYNTAX: reverse-direction `sbx cp <name>:<remote-path>
      // <local-path>` — same caveat as _copyIn above.
      const remoteSrc = `${handle.sandbox_id}:${declaredPath}`;
...
export const _internal = { isSafeRelativeOutputPath, sandboxNameFor, sbxHome, scratchRoot };
