...
export const CODEX_HARNESS = validateHarnessSpec({
  ...
  // `codex exec --json` emits newline-delimited lifecycle events. Its final
  // turn.completed event currently carries token counts, but no model or
  // duration; Federation measures the execution window itself and records
  // unavailable fields as null rather than guessing.
  entrypoint: 'codex exec --json --dangerously-bypass-approvals-and-sandbox',
  ...
  login_status_probe: {
    ...
    success: { SBX_CRED_OPENAI_MODE: 'oauth' },
  },
 ...
 *    otherwise-wasted SUBSCRIPTION capacity, not paying per-token). Verified
 *    live on 2026-07-21: `claude auth login --claudeai`, run through `sbx
 *    exec` in the job sandbox, prints a capturable OAuth URL after "visit:"
 *    and `claude auth status --json` reports `loggedIn:true` on completion.
 *    This is a scriptable host-url-flow; no attached terminal or `/login`
 *    command is required in the normal path.
 ...
 *
 * The subscription harness proves its active mode with `claude auth status
 * --json` (`loggedIn:true`); the API-key variant uses the sbx proxy's
 * `SBX_CRED_ANTHROPIC_MODE=apikey` signal. These are explicit mode checks,
 * not a bare "request succeeded" inference. Waspflow's own `lib/billing.sh`
 * already treats a stray ANTHROPIC_API_KEY silently overriding subscription
 * auth as a real, previously-encountered risk for headless Claude workers.
 */
const CLAUDE_SUBSCRIPTION_STATUS_PROBE = {
  command: 'claude auth status --json',
  reports_auth_mode: true,
  mode_field_hint: 'JSON field `loggedIn:true` is Claude Code\'s non-interactive success signal for the Claude.ai subscription login.',
...
const CLAUDE_API_KEY_STATUS_PROBE = {
  ...
  success: { SBX_CRED_ANTHROPIC_MODE: 'apikey' },
};
...
export const CLAUDE_CODE_SUBSCRIPTION_HARNESS = validateHarnessSpec({
  ...
  // Probed from the installed Claude CLI's --help: --verbose plus
  // --output-format stream-json emits newline-delimited assistant/tool/result
  // events.  This is the agent's own session transcript, not wrapper stderr.
  entrypoint: 'claude --print --verbose --output-format stream-json --dangerously-skip-permissions',
  ...
  cancellation: { cancel_signal: 'sbx stop <sandbox>', on_cancel: 'sandbox process terminated; sandbox state preserved until sbx rm' },
  result_behavior: { result_transport: 'native Claude stream-json event stream' },
});

export const CLAUDE_CODE_API_KEY_HARNESS = validateHarnessSpec({
  harness_id: 'claude-code-api-key',
  install: 'claude', // built-in sbx agent template name
  entrypoint: 'claude --print --verbose --output-format stream-json --dangerously-skip-permissions', // same external-sandbox rationale as CLAUDE_CODE_SUBSCRIPTION_HARNESS above
  pinned_cli_version: 'unpinned — see profiles/wf-federation-docker-v0.json for the sbx floor; Claude Code version pinning is a separate follow-up',
  provider_service_id: 'anthropic',
  provider_domains: ['api.anthropic.com', 'claude.ai'],
  auth_header: { header: 'Authorization', format: 'Bearer %s' },
...
export const GEMINI_HARNESS = validateHarnessSpec({
  ...
  login_status_probe: {
    ...
    success: { SBX_CRED_GOOGLE_MODE: 'apikey' },
  },
  cancellation: { cancel_signal: 'sbx stop <sandbox>', on_cancel: 'sandbox process terminated; sandbox state preserved until sbx rm' },
