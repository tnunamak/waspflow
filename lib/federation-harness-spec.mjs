 ...
 * @property {{command: string, reports_auth_mode: boolean, mode_field_hint: string, success: object}} login_status_probe
...
export function validateHarnessSpec(spec) {
  ...
  isPlainObject(spec.login_status_probe.success) || fail('login_status_probe.success must be an object of expected probe fields');
  Object.keys(spec.login_status_probe.success).length > 0 || fail('login_status_probe.success must name at least one expected probe field');
...
function validateCredentialDiscovery(strategy, discovery) {
  ...
  switch (strategy) {
    case 'docker-native-oauth':
      ...
      // mediate. The normal path is a browser URL; the second shape remains
      // only for a genuinely non-scriptable fallback:
      //   'host-url-flow' — the command prints a URL, and completes when
      //     the user finishes in a browser (Codex: `sbx
      ...
      //   'interactive-session-flow' — rare fallback for a harness that
      //     really cannot print a browser URL. Shipped harnesses should use
      //     host-url-flow whenever their CLI exposes a scriptable login.
