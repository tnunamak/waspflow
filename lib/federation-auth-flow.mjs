 ...
 *   (b) Detect-first: check whether the provider secret is already set in
 *       the Waspflow-scoped sbx profile before triggering anything. Only
 *       start a login flow when a job actually needs a provider that isn't
 *       authed yet. Never re-prompt once set.
 ...
 *
 * Scope limit, stated honestly rather than papered over: this module drives
 * `docker-native-oauth` + `host-url-flow` harnesses (Codex today). It does
 * NOT and cannot reduce Claude Code's `interactive-session-flow` to the same
 * {url, waitForCompletion} shape — `/login` must run inside an attached,
 * interactive sandbox session, and there is no host-side URL to capture. See
 * `describeAuthRequirement()`'s honest branch for that case.
 */
 ...
 * @property {string} [url]            present once parsed from the command's stdout
 ...
 * Waspflow-scoped sbx profile, WITHOUT triggering any login flow. Callers
 * must check this before startAuthFlow() — starting a flow for an
 * already-authed provider would violate the "never re-prompt once set"
 * requirement.
 *
 ...
 *
 * @param {import('./federation-harness-spec.mjs').HarnessSpec} harnessSpec
 * @param {{sbxBin?: string, env?: NodeJS.ProcessEnv, urlTimeoutMs?: number, completionTimeoutMs?: number}} [opts]
 * @returns {AuthFlowHandle}
 */
export function startAuthFlow(harnessSpec, opts = {}) {
  ...
  if (harnessSpec.credential_discovery.flow_shape !== 'host-url-flow') {
    throw new AuthFlowError(
      ...
      `attached interactive sandbox session (see describeAuthRequirement() for the honest description of that case).`
    );
  }

  const sbxBin = opts.sbxBin || process.env.WASPFLOW_SBX_BIN || 'sbx';
  const env = opts.env || process.env;
  const urlTimeoutMs = opts.urlTimeoutMs ?? 15_000;
  const completionTimeoutMs = opts.completionTimeoutMs ?? 5 * 60_000;

  // credential_discovery.login_command is DOCUMENTATION of the real sbx
  // invocation (e.g. 'sbx secret set -g openai --oauth'), not a literal
  // shell command to run — the actual binary is always `sbxBin` (so tests
  // and WASPFLOW_SBX_BIN overrides are honored); only the ARGUMENTS after
  // the leading 'sbx' are taken from it.
  const [, ...args] = harnessSpec.credential_discovery.login_command.split(' ');
  ...
  const completion = new Promise((resolve) => {
    ...
    child.stdout.on('data', (chunk) => {
      ...
      if (!handle.url && urlPattern.test(stdoutBuf)) {
        const match = urlLineExtract.exec(stdoutBuf);
        if (match) {
          handle.url = match[0];
    ...
    child.on('exit', (code) => {
      ...
      if (code === 0) {
        ...
        settle({ status: 'complete', detail: stdoutBuf.trim() });
      } else {
        handle.status = 'failed';
  ...
  handle.waitForCompletion = async () => {
    const timeoutAt = handle.url ? completionTimeoutMs : urlTimeoutMs;
    ...
    const timeoutPromise = delay(timeoutAt, undefined, { signal: timeoutController.signal })
      .then(() => {
        const reason = handle.url
          ? `timed out after ${timeoutAt}ms waiting for the browser step to complete`
          : `timed out after ${timeoutAt}ms waiting for a login URL to appear`;
        handle.cancel();
