...
export function parseHarnessExecutionResult(harnessId, stdout) {
  ...
  if (harnessId === 'claude-code-subscription' || harnessId === 'claude-code-api-key') {
    // Claude stream-json ends in a result event. Keep accepting the older
    // one-object --output-format json shape so already persisted runs remain
    // readable after the harness moves to agent-native streaming.
    const events = output.split(/\r?\n/).map(parseJson).filter(Boolean);
    const result = events.findLast((event) => event.type === 'result') || parseJson(output) || {};
    const models = result?.modelUsage && typeof result.modelUsage === 'object' ? Object.keys(result.modelUsage) : [];
    // Claude has emitted both a direct model field and modelUsage across CLI
    // versions. A multi-model turn is real execution metadata, not an error:
    // preserve every reported model instead of discarding the receipt because
    // it is not a single-item array.
    const directModel = [result?.model, result?.model_name, result?.metadata?.model, result?.usage?.model]
      .find((value) => typeof value === 'string' && value.trim());
    return {
      model: directModel || (models.length ? models.join(', ') : null),
      usage: nullableUsage(result?.usage?.input_tokens ?? result?.usage?.inputTokens, result?.usage?.output_tokens ?? result?.usage?.outputTokens),
    };
  }
  if (harnessId === 'codex') {
    const events = output.split(/\r?\n/).map(parseJson).filter(Boolean);
 ...
 */
export async function probeFederationIdentity({ harnessNames = ['claude-code-subscription', 'codex', 'gemini', 'gh-cli'], backendFactory = () => new DockerSbxBackend() } = {}) {
  ...
  for (const name of harnessNames) {
    ...
    try {
      ...
      providers.push({
        ...
        capacity_kind: harness.provider_service_id === 'github' ? 'n/a' : capacityKindForHarness(harness),
        ...(account?.email ? { account_email: account.email } : {}),
        ...(account?.tier ? { tier: account.tier } : {}),
        authed: account !== null,
      });
    } catch {
      providers.push({ service: harness.provider_service_id, capacity_kind: harness.provider_service_id === 'github' ? 'n/a' : capacityKindForHarness(harness), authed: false });
    } finally {
 ...
 */
export function buildValidatedJobSpec({ taskDigest, harness, entrypointWithPrompt, githubAccessRequired = false, resources = DEFAULT_RESOURCES }) {
  const spec = {
    ...
    ...(githubAccessRequired ? { github_access_required: true } : {}),
  };
 ...
 */
export async function runJob({ jobSpec, materializeSource, cloneGitSource, authorize, statusProbe, onLog, now = () => new Date(), backend = new DockerSbxBackend() }) {
  const capabilities = await backend.probeCapabilities();
  ...
  try {
    ...
    if (cloneGitSource) {
      if (typeof backend.cloneGitSource !== 'function') throw new Error('sandbox backend does not support Git repository task sources');
      await backend.cloneGitSource(handle, cloneGitSource);
    }
    ...
    const startedAt = now();
    await backend.start(handle, { onLog });
