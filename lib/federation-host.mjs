...
export function normalizeNgrokDomain(value) {
  ...
  if (!/^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)+$/.test(domain)) {
    throw new FederationHostError('the permanent address must be a domain name from ngrok');
  }
  return domain;
}
...
export function saveCollectiveName(config, collectiveName) {
  const name = String(collectiveName || '').trim();
  if (!name) throw new FederationHostError('collective name must not be empty');
  const next = { ...config, collective_name: name };
  saveJsonFile(path.join(path.dirname(config.roster_path), 'host.json'), next);
  return next;
}

/** Persists a moved coordinator port (chosen when the configured one is taken). */
export function savePort(config, port) {
