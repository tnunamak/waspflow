...
export function createJoinInvite({ coordinatorUrl, collectiveToken, collectiveName }) {
  const invite = new URL('waspflow://join');
  invite.searchParams.set('coordinator', normalizeCoordinatorUrl(coordinatorUrl));
  invite.searchParams.set('token', collectiveToken);
  if (collectiveName) invite.searchParams.set('name', collectiveName);
