...
export async function startNgrokTunnel({ port, authtoken, domain, loadSdk = loadCoordinatorNgrokSdk }) {
  ...
  // A configured domain is the only address requested across restarts. If
  // ngrok rejects it, retry without one so the collective comes back
  // reachable; the caller surfaces the changed URL to the operator.
  try {
