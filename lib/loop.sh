...
# Drive ONE semantic agent: spawn --report, wait, read report, reap. Echoes report content.
# $1 lane  $2 provider  $3 report-path  $4 prompt
loop_run_agent() {
  local lane="$1" provider="$2" report="$3" prompt="$4"
  "$_LOOP_DIR/../bin/waspflow" spawn --provider "$provider" --lane "$lane" --report "$report" -- "$prompt" >&2 || return 1
...
loop_run() {
  ...
  local cls; cls="$(loop_run_agent "loop-classify-$rid" "$provider" "$clsrep" "$(profile_classify_prompt "$raw" "$intent")")"
...
try: d=json.loads(sys.stdin.read()); print(len(d.get("hardAreasInspected",[])))
  ...
  local tfile tsym; tfile="$(printf '%s' "$target" | _loop_jget file)"; tsym="$(printf '%s' "$target" | _loop_jget symbol)"
  ...
  for attempt in 1 2 3; do
    ...
    local mout; mout="$(loop_run_agent "loop-make-$rid-$attempt" "$provider" "$makerep" "$mprompt")"
    ...
    facts="$(oracle_gate "$wt" "$branch" "$tfile" "$tsym" "$testcmd_file" "$baseline" "$(profile_lint_file_cmd "$tfile")")"
    ...
    sem="$(loop_run_agent "loop-check-$rid-$attempt" "${LOOP_CHECKER_PROVIDER:-$provider}" "$semrep" "$(profile_check_prompt "$target" "$branch" "$facts" "$semrep")")"
...
try: d=json.loads(sys.stdin.read()); print(", ".join(f["symbol"] for f in d.get("classified",[]) if f.get("classification")!="safe-incidental")[:200])
    ...
    done="$(loop_run_agent "loop-done-$rid" "${LOOP_CHECKER_PROVIDER:-$provider}" "$donerep" "$(profile_done_prompt "$target" "$facts" "$hardlist" "$intent" "$donerep")")"
