...
#
# Roots are searched LIVE FIRST and the archive is only reached when live yields
# nothing. That ordering is deliberate on cost grounds: at spawn time the file is
# seconds old and always live, so the common path never pays to walk the archive
...
_codex_find_rollout_for_submitted_prompt() {
  ...
  if [[ "$marker" == WASPFLOW_LANE_MARKER:* ]]; then
    ...
    listing="$(_codex_rollout_listing)"
  fi
  while IFS= read -r f; do
    [[ -n "$f" ]] || continue
    fcwd="$(head -1 "$f" 2>/dev/null | jq -rc 'select(.type=="session_meta") | .payload.cwd // empty' 2>/dev/null)"
