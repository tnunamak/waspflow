...
function submitBody(body) {
  ...
  return { source: body.source?.trim() || '', prompt: body.prompt, displayId: body.display_id.trim(), network: body.network || 'disabled', attachments };
}
  ...
  async function superviseSubmit(body) {
    ...
    try {
      child = spawnProcess(process.execPath, [
        ...
        '--source', sourcePath, '--prompt', input.prompt, '--network', input.network,
