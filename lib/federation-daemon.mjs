...
function submitBody(body) {
  ...
  if (body.git_private !== undefined && typeof body.git_private !== 'boolean') throw new DaemonRequestError('submit git_private must be boolean when provided', 400);
  ...
  return { source: body.source?.trim() || '', gitUrl, gitRef: body.git_ref?.trim() || '', gitPrivate: body.git_private === true, prompt: body.prompt, displayId: body.display_id.trim(), network: gitUrl ? 'enabled' : body.network || 'disabled', attachments };
}
...
export async function startFederationDaemon({
  ...
  async function requireGitHubTaskAccess(taskDigest) {
    ...
    if (!task?.git_source?.authentication_required) return;
  ...
  async function superviseSubmit(body) {
    ...
    try {
      child = spawnProcess(process.execPath, [
        ...
        ...(input.gitUrl ? ['--git-url', input.gitUrl, '--git-private', String(input.gitPrivate), ...(input.gitRef ? ['--git-ref', input.gitRef] : [])] : ['--source', sourcePath]),
