...
export async function startFederationDaemon({
  ...
  async function identity() {
    ...
    if (!identityCache || current - identityCache.at >= IDENTITY_CACHE_MS) {
      const observed = await identityProbe();
      identityCache = { at: current, observed };
    }
    const currentConfig = config();
    return {
      docker_account: identityCache.observed.docker_account || null,
      providers: Array.isArray(identityCache.observed.providers) ? identityCache.observed.providers : [],
  ...
  function superviseSubmit(body) {
    ...
    child.once('error', (error) => {
      if (submitChild === child) {
        submitChild = null;
        submission = { ...submission, state: 'failed', detail: 'Submission could not be started.' };
      }
    });
    child.once('close', (code) => {
      ...
      submission = {
        ...
        detail: code === 0 ? 'Submission settled. Your result is ready to review.' : 'Submission could not be completed.',
      };
    });
    return true;
  }
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/identity') return sendJson(response, 200, await identity());
