...
export async function startFederationDaemon({
  ...
  function superviseContribute(taskDigest = null) {
    ...
    child.once('close', (code) => {
      ...
      if (executionLog && contribution) contribution = { ...contribution, execution_log: executionLog };
  ...
  function readExecutionLog(taskDigest) {
    ...
    return {
      ...
      output,
      truncated: output.startsWith('[Waspflow: earlier output was truncated'),
    };
  }
  ...
  async function readTaskDetail(taskDigest) {
    ...
    return {
      ...body,
      receipt,
      execution_log_available: existsSync(executionLogPath(executionLogsDir, taskDigest)),
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === 'GET' && taskLogMatch) return sendJson(response, 200, readExecutionLog(taskLogMatch[1]));
