...
function loadSettings(path) {
  if (!existsSync(path)) return { schedule: { enabled: false, start: '', end: '', days: '' } };
  try {
    const parsed = JSON.parse(readFileSync(path, 'utf8'));
...
function settingsBody(body) {
  ...
  if (typeof enabled !== 'boolean' || typeof start !== 'string' || typeof end !== 'string' || typeof days !== 'string') {
    throw new DaemonRequestError('schedule requires boolean enabled and string start, end, and days values', 400);
  }
  if (start.length > 16 || end.length > 16 || days.length > 120) throw new DaemonRequestError('schedule values are too long', 400);
  return { schedule: { enabled, start, end, days } };
}
...
export async function startFederationDaemon({
  ...
  function identity() {
    ...
    return {
      ...
      ...(currentConfig?.collective_name ? { collective_name: currentConfig.collective_name } : {}),
  ...
  function superviseSubmit(body) {
    ...
    let sourceInfo;
    try { sourceInfo = statSync(input.source); } catch { throw new DaemonRequestError('The source folder does not exist. Choose an existing folder on this computer.', 400); }
