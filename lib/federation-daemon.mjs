...
import { configHome, leaveCollective, loadConfig, switchCollective } from './federation-config.mjs';
import { joinFederation as joinFederationOperation } from './federation-join.mjs';
...
const PENDING_APPROVAL_DETAIL = 'Your request is with the collective operator; you can close this — contributions start once you’re approved.';
...
/**
 * Ask the already-authenticated local daemon to read the current member
 * config. Returns 'reloaded', 'failed' (a daemon is registered but did not
 * accept the reload — the caller should tell the user to restart it), or
 * 'no_daemon' (nothing running, so no user action is needed).
 */
export async function reloadRunningFederationDaemon({ infoPath = daemonInfoPath(), fetchImpl = fetch } = {}) {
  const info = readDaemonInfo(infoPath);
  if (!info) return 'no_daemon';
  try {
    const response = await fetchImpl(`http://127.0.0.1:${info.port}/reload`, {
      method: 'POST',
      headers: { 'x-waspflow-session-token': info.token },
...
/** Whether the local daemon has collective-bound work that cannot be switched safely. */
export async function federationDaemonIsBusy({ infoPath = daemonInfoPath(), fetchImpl = fetch } = {}) {
  const info = readDaemonInfo(infoPath);
  if (!info) return false;
  try {
    const response = await fetchImpl(`http://127.0.0.1:${info.port}/status`, {
      headers: { 'x-waspflow-session-token': info.token },
      signal: AbortSignal.timeout(500),
    });
    if (!response.ok) return false;
    const body = await response.json();
    return ['contributing', 'pausing', 'action_needed'].includes(body.state)
      || ['pending', 'published'].includes(body.submission?.state);
  } catch {
    return false;
  }
}
...
export { parseJoinInvite } from './federation-invite.mjs';
...
function createApprovalPoller({ config, fetchImpl, intervalMs, recordCoordinator, coordinatorUnavailable, getState, setState, isContributing, pauseAfterCurrent }) {
  let timer = null;
  ...
  async function poll() {
    ...
    if (!currentConfig || requestInFlight) return;
    ...
    try {
      ...
      if (approved) {
        ...
        wasGranted = true;
        if (getState() === 'pending_approval' || getState() === 'not_joined' || (getState() === 'approval_revoked' && !isContributing())) setState('idle');
      } else if (wasGranted) {
        pauseAfterCurrent();
        setState('approval_revoked', isContributing()
          ? 'Approval was revoked. The current task may finish, then Waspflow will pause.'
          : APPROVAL_REVOKED_DETAIL);
      } else if (getState() !== 'contributing' && getState() !== 'paused') {
  ...
  function observe() {
    ...
    if (activeIdentity !== identity) {
      ...
      approvedIdentity = null;
      wasGranted = false;
      if (getState() !== 'contributing' && getState() !== 'paused') setState('pending_approval', PENDING_APPROVAL_DETAIL);
    }
    if (approvedIdentity === identity) return true;
    if (!timer) {
  ...
  return { observe, reset, stop };
}

function createCoordinatorHealth({ now }) {
  let unavailable = false;
  let lastSuccessAt = null;
  let schemaVersion = null;
  let outdated = false;

  function observeSchema(response) {
    const value = Number(response?.headers?.get?.(COORDINATOR_SCHEMA_HEADER));
    if (Number.isSafeInteger(value) && value > 0) {
      schemaVersion = value;
      outdated = value < REQUIRED_COORDINATOR_SCHEMA_VERSION;
    }
  }

  function record(response) {
    unavailable = !response.ok;
    if (response.ok) lastSuccessAt = now().toISOString();
    observeSchema(response);
  }
  ...
  function snapshot() {
    return {
      coordinator_unavailable: unavailable,
      ...(lastSuccessAt ? { coordinator_last_success_at: lastSuccessAt } : {}),
      ...(schemaVersion !== null ? { coordinator_schema_version: schemaVersion } : {}),
      ...(outdated ? { coordinator_outdated: true } : {}),
    };
  }

  return { markUnavailable, observeSchema, record, reset, snapshot };
}

function createIdentityTracker({ config, collectiveName, identityProbe, now, cacheMs }) {
  let cache = null;
  let refreshInFlight = null;
  let refreshTimer = null;
  let probeFailed = false;
  let providers = [];
  const confirmedProviders = new Map();
  const negativeCounts = new Map();

  function updateProviders(observedProviders) {
    const next = [];
    for (const observed of Array.isArray(observedProviders) ? observedProviders : []) {
      ...
      if (authenticated === true) {
        ...
        confirmedProviders.set(service, confirmed);
        negativeCounts.delete(service);
        next.push(confirmed);
        continue;
      }
      const confirmed = confirmedProviders.get(service);
      const negatives = (negativeCounts.get(service) || 0) + 1;
      negativeCounts.set(service, negatives);
      if (confirmed && negatives < IDENTITY_NEGATIVE_CONFIRMATIONS) {
        next.push({ ...confirmed, checking: true });
      } else {
        if (negatives >= IDENTITY_NEGATIVE_CONFIRMATIONS) confirmedProviders.delete(service);
        next.push({ ...observed });
      }
    }
    ...
    for (const [service, confirmed] of confirmedProviders) {
  ...
  function refresh() {
    ...
    refreshInFlight = Promise.resolve(identityProbe())
      .then((observed) => {
        cache = { at: now().getTime(), observed };
        updateProviders(observed?.providers);
        probeFailed = false;
      })
      .catch(() => {
        cache = { at: now().getTime(), observed: cache?.observed || {} };
        probeFailed = true;
      })
  ...
  function snapshot() {
    ...
    const stale = !cache || current - cache.at >= cacheMs;
    if (stale) void refresh();
    const currentConfig = config();
    const observed = cache?.observed || {};
    return {
      docker_account: observed.docker_account || null,
      docker_status: observed.docker_account ? 'detected' : refreshInFlight ? 'checking' : probeFailed ? 'failed' : 'not_reported',
      providers: providers.map((provider) => {
        const checking = Boolean(provider.checking || refreshInFlight || probeFailed);
      ...
      refreshing: Boolean(refreshInFlight || stale),
    };
  }
  ...
  function start() {
    void refresh();
    refreshTimer = setInterval(() => { void refresh(); }, cacheMs);
    refreshTimer.unref?.();
  }
  ...
  return { invalidate, refresh, snapshot, start, stop };
}
...
export async function startFederationDaemon({
  ...
  joinFederation = joinFederationOperation,
  switchFederationCollective = switchCollective,
  leaveFederationCollective = leaveCollective,
} = {}) {
  ...
  const coordinatorHealth = createCoordinatorHealth({ now });
  const identityTracker = createIdentityTracker({ config, collectiveName, identityProbe, now, cacheMs: IDENTITY_CACHE_MS });

  const approvalPoller = createApprovalPoller({
    config,
    fetchImpl,
    intervalMs: approvalPollIntervalMs,
    recordCoordinator: coordinatorHealth.record,
    coordinatorUnavailable: coordinatorHealth.markUnavailable,
    getState: () => state,
    setState,
    isContributing: () => Boolean(contributeChild),
    pauseAfterCurrent: () => { pauseAfterCurrent = true; },
  });
  ...
  function reloadCollectiveState() {
    approvalPoller.reset();
    coordinatorHealth.reset();
    taskDisplayIds.clear();
    taskRequesters.clear();
    taskDetails.clear();
    submission = null;
    identityTracker.invalidate();

    // A local name describes the previous collective. Let the new invite name
    // win instead of carrying that stale label across a switch.
    if (settings.collective_name) {
      settings = { ...settings };
      delete settings.collective_name;
      saveSettings(settingsPath, settings);
    }

    const currentConfig = config();
    if (contributeChild) return status();
    setState(approvalConfig(currentConfig) ? 'pending_approval' : (currentConfig ? 'idle' : 'not_joined'));
    approvalPoller.observe();
    void identityTracker.refresh();
    return status();
  }

  function collectiveSwitchBlocker() {
    return contributeChild || submitChild || providerLoginFlow || providerLoginStarting || dockerLoginFlow || dockerLoginStarting || dockerLoginResume
      ? 'Finish the current work before switching collectives.'
      : null;
  }

  function collectives() {
    const currentConfig = config();
    const { coordinator_unavailable: coordinatorUnavailable } = coordinatorHealth.snapshot();
    return (currentConfig?.collectives || []).map(({ id, coordinator_url, collective_name, key_id, joined_at, last_seen }) => {
      const active = id === currentConfig.active_collective_id;
      return { id, coordinator_url, collective_name: collective_name || null, key_id, joined_at, last_seen, active, reachable: active ? !coordinatorUnavailable : null, state: active ? (coordinatorUnavailable ? 'unreachable' : 'reachable') : 'inactive' };
    });
  }

  function recordContribution({ event = null, outcome, reason = null, requesterNotice = null } = {}) {
    const currentConfig = contributionConfig || config();
  ...
  function status() {
    ...
    if (state === 'pending_approval' && typeof currentConfig?.approval_request === 'string') result.approval_request = currentConfig.approval_request;
    if (action) result.action = action;
    if (submission) result.submission = submission;
    if (contribution) result.contribution = contribution;
    Object.assign(result, coordinatorHealth.snapshot());
  ...
  function superviseContribute(taskDigest = null) {
    ...
    if (!approvalPoller.observe()) return false;
    contributionConfig = { ...startingConfig };
    pauseAfterCurrent = false;
  ...
  async function listRequests() {
    ...
    const body = await response.json().catch(() => null);
    coordinatorHealth.observeSchema(response);
  ...
  async function requireGitHubTaskAccess(taskDigest) {
    ...
    const github = identityTracker.snapshot().providers.find((provider) => provider.service === 'github');
  ...
  function superviseJoin(invite) {
    if (joinChild || collectiveSwitchBlocker()) return false;
    setState('not_joined', 'Joining the federation…');
    joinChild = { pending: true };
    void Promise.resolve(joinFederation({ invite })).then(() => {
      reloadCollectiveState();
    }).catch((error) => setState(config() ? 'idle' : 'not_joined', error.message || 'Join could not be completed.')).finally(() => { joinChild = null; });
    return true;
  }
  ...
  async function superviseSubmit(body) {
    ...
    if (coordinatorHealth.snapshot().coordinator_outdated && input.githubAccessRequired) {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/collectives') return sendJson(response, 200, { collectives: collectives() });
      const collectiveMatch = /^\/collectives\/([^/]+)\/(switch|leave)$/.exec(url.pathname);
      if (request.method === 'POST' && collectiveMatch) {
        const [, id, operation] = collectiveMatch;
        const currentConfig = config();
        const active = currentConfig?.active_collective_id === id;
        const blocked = (operation === 'switch' || active) && collectiveSwitchBlocker();
        if (blocked) throw new DaemonRequestError(blocked, 409);
        if (operation === 'switch') switchFederationCollective(id);
        else leaveFederationCollective(id);
        return sendJson(response, 200, operation === 'switch' || active ? reloadCollectiveState() : status());
      }
      if (request.method === 'POST' && url.pathname === '/reload') {
        const blocked = collectiveSwitchBlocker();
        if (blocked) throw new DaemonRequestError(blocked, 409);
        return sendJson(response, 200, reloadCollectiveState());
      }
      if (request.method === 'GET' && url.pathname === '/identity') return sendJson(response, 200, identityTracker.snapshot());
      ...
      if (request.method === 'POST' && url.pathname === '/contribute/start') {
        ...
        if (!approvalPoller.observe()) throw new DaemonRequestError(PENDING_APPROVAL_DETAIL, 409);
...
/**
 * The background Federation daemon must not inherit the desktop session bus.
 * Its sandbox credential daemon is long-lived, so stripping D-Bus only from
 * individual `sbx` commands is too late: sandboxd may already have selected
 * the operator's desktop Secret Service as its credential store.
 */
export function federationDaemonEnv(baseEnv = process.env) {
  const env = { ...baseEnv };
...
export async function openFederationUi({ spawnProcess = spawnChild, cliPath = DEFAULT_CLI_PATH, infoPath = daemonInfoPath(), platformName = platform() } = {}) {
  ...
  if (!info || !(await daemonIsReachable(info))) {
    const child = spawnProcess(process.execPath, [cliPath, 'daemon'], {
      detached: true,
      stdio: 'ignore',
      env: federationDaemonEnv(),
    });
    child.unref?.();
