...
function emptySettings() {
  return { schedule: { enabled: false, start: '', end: '', days: '' } };
}
...
function submitBody(body) {
  ...
  return { source: body.source?.trim() || '', prompt: body.prompt, displayId: body.display_id };
}
...
function settingsBody(body) {
  ...
  const { enabled, start = '', end = '', days = '' } = body.schedule;
  const collectiveName = body.collective_name;
  if (typeof enabled !== 'boolean' || typeof start !== 'string' || typeof end !== 'string' || typeof days !== 'string' || (collectiveName !== undefined && typeof collectiveName !== 'string')) {
    throw new DaemonRequestError('schedule requires boolean enabled and string start, end, and days values', 400);
  }
  if (start.length > 16 || end.length > 16 || days.length > 120 || collectiveName?.length > 120) throw new DaemonRequestError('settings values are too long', 400);
  return { ...(typeof collectiveName === 'string' ? { collective_name: collectiveName.trim() } : {}), schedule: { enabled, start, end, days } };
}

const IDENTITY_HARNESS_BY_SERVICE = Object.freeze({ openai: 'codex', anthropic: 'claude-code-subscription' });

async function startProviderSignIn(service) {
  ...
  try {
    ...
    const flow = await startAuthFlow(harness, { sandboxId: handle.sandbox_id });
...
export async function startFederationDaemon({
  ...
  startProviderSignIn: providerSignInStarter = startProviderSignIn,
  ...
  async function beginProviderLogin(service) {
    if (providerLoginFlow || dockerLoginFlow) throw new DaemonRequestError('A sign-in is already in progress.', 409);
    let flow;
    try {
      flow = await providerSignInStarter(service);
    } catch (error) {
      throw error instanceof DaemonRequestError ? error : new DaemonRequestError(error.message || 'Provider sign-in could not be started.', 502);
    }
  ...
  function reflectContributeEvent(event, exitCode, stderr = '') {
    ...
    if (!event) {
      const childDetail = lastStderrLine(stderr);
      setState('idle', exitCode === 0 ? 'Contribution finished.' : childDetail || `Contribution stopped (exit ${exitCode ?? 'unknown'}).`);
      return;
    }
    if (event.type === 'awaiting_browser') {
      ...
      setState('idle', `Finished '${lastCompleted?.display_id || 'a task'}'.`);
    } else {
  ...
  function superviseContribute(taskDigest = null) {
    ...
    child.once('close', (code) => {
      ...
      contributeChild = null;
      reflectContributeEvent(finalEvent(stdout), code, stderr);
      resumeAfterDockerLogin();
    });
    return true;
  }
  ...
  async function readTaskDetail(taskDigest) {
    ...
    return { ...body, receipt, ...(execution ? { execution_metadata: execution } : {}) };
  }

  function refreshIdentity() {
    if (identityRefreshInFlight) return identityRefreshInFlight;
    identityRefreshInFlight = Promise.resolve(identityProbe())
      .then((observed) => { identityCache = { at: now().getTime(), observed }; identityProbeFailed = false; })
      .catch(() => { identityCache = { at: now().getTime(), observed: {} }; identityProbeFailed = true; })
  ...
  function identity() {
    ...
    return {
      ...
      providers: Array.isArray(observed.providers) ? observed.providers : [],
  ...
  async function superviseSubmit(body) {
    ...
    submission = { state: 'submitting', detail: 'Preparing and publishing your task.', task_digest: null, display_id: input.displayId, published_at: null };
    let child;
    try {
      child = spawnProcess(process.execPath, [
        cliPath, 'submit', '--display-id', input.displayId,
        '--source', sourcePath, '--prompt', input.prompt,
    ...
    child.once('error', (error) => {
      if (submitChild === child) {
        submitChild = null;
        submission = { ...submission, state: 'failed', detail: error.message || 'Submission could not be started.' };
      }
    });
    child.once('close', (code) => {
      ...
      submission = {
        ...
        detail: code === 0 ? 'Submission settled. Your result is ready to review.' : lastStderrLine(stderr) || 'Submission could not be completed.',
      };
      if (temporarySourcePath) void rm(temporarySourcePath, { recursive: true, force: true });
    });
    return true;
