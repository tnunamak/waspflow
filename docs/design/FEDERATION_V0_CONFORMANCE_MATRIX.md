...
| J. Native Docker auth substrate, no custom gateway | `lib/federation-docker-backend.mjs` and `lib/federation-harnesses.mjs` never reference a custom base-URL/gateway/proxy-shaped pattern for Codex/Claude/gh-cli (static regression guard, precise enough to avoid tripping on this file's own "not a gateway" documentation prose); the per-harness live proof script exists, is syntactically valid, and resolves all three `HarnessSpec` instances; the six-column matrix (existing host login, extra login required, credential stays outside VM, refresh works, subscription allowance used, full CLI runs in VM) is exercised per-harness by an operator running `scripts/federation-harness-auth-proof-live-run.sh {codex\|claude-code\|gh-cli}` | `bash tests/federation-docker-conformance.sh` (static half runs today); `bash scripts/federation-harness-auth-proof-live-run.sh codex` / `claude-code` / `gh-cli` (live half, requires sbx + interactive host logins; "refresh works" additionally needs a long-duration follow-up past a real token's expiry window) | **Static check PASSES today** (verified adversarially with three distinct injected violations — a real `ANTHROPIC_BASE_URL` reference, a `base_url:` assignment, a `new *Gateway(` call — each caught, and reverting cleanly to a pass once removed). **Live proof SKIPs** for all three harnesses — no sbx on this machine, and even with sbx the live half requires interactive host logins this environment cannot perform. See `docs/design/FEDERATION_V0_UAT_REPORT.md`'s "Auth architecture" section for the full per-harness matrix and the two dangerous assumptions this gate exists to rule out. |
