...
| H. Version-pinned conformance testing | A supported `sbx` version-range mechanism exists and structurally rejects/warns on an unpinned or out-of-range version, tested with a stub `sbx` reporting a bogus version | `bash tests/federation-docker-conformance.sh` (no live sbx or root required — this gate is fully runnable today once a detection mechanism exists) | **SKIP — detection not yet implemented.** Neither `bin/federation-detect-sbx` nor `profiles/wf-federation-docker-v0.json` exists in this checkout as of this suite's authorship. The suite's `gate_h_version_pinned_conformance` function and stub-`sbx`-on-PATH harness are ready to run for real the moment either lands. |
