...
description: >-
  Spawn, watch, steer, and reap Claude/Codex/Grok/Antigravity/Qwen coding-agent workers live in
...
Codex, Grok, Antigravity, or Qwen Code) in tmux windows you watch and steer, then reap. Lane state is on
...
Claude, Grok, Antigravity, and Qwen need no backend gate. First-time user: `waspflow demo --provider
...
on serious lanes. `waspflow receipts summary` shows the ledger: per-arm
outcomes and the eligible fraction.
