...
mkdir -p "$stage_dir/usr/lib/waspflow/bin" \
  ...
  "$stage_dir/usr/lib/waspflow/coordinator" \
...
# This manifest is tiny and makes the SDK an explicit coordinator-scoped
# dependency. Its node_modules are deliberately *not* staged: only `host`
# installs them, after the operator selects ngrok.
install -m 0644 "$repo_root"/coordinator/package.json "$repo_root"/coordinator/package-lock.json "$stage_dir/usr/lib/waspflow/coordinator/"
...
chmod go-w "$package_dir/assets/waspflow-federation-daemon.service" "$package_dir/assets/waspflow-federation-coordinator.service" "$package_dir/assets/waspflow-federation-tray.desktop"
