...
# Descendant-process ownership (2026-07-14 incident): five pytest processes
# launched from completed/reaped lanes survived ~26h reparented to systemd
# --user, no TTY/no tmux ancestor, because ownership only ever covered the
# tmux pane PID. These tests drive the REAL cmd_spawn/tmux_create_owned_lane_window
# (which wraps the pane's shell_command in a transient `systemd-run --user
# --scope`) and the REAL reap path, on a private, uniquely named tmux session +
# a real (but throwaway, user-scoped) systemd unit — never the operator's tmux
# server or any live lane. Skipped entirely on hosts without a usable systemd
# --user instance so verify.sh stays green in minimal/container environments.
...
scopep_preflight() { :; }
scopep_discover_session() { echo x; }
...
# The lane "command" is whatever the test passes as the prompt, run directly as
# shell so tests can construct grandchild/timeout/detach scenarios precisely.
scopep_spawn() {
  local lane="$1" cwd="$2" _model="$3" _sid="$4" _transcript="$5" prompt="$6"
  tmux_create_owned_lane_window "$lane" "$cwd" "exec bash -c $(printf '%q' "$prompt")" >/dev/null || return 1
}
PROV
  ...
  scopework="$(mktemp -d "$scratch/waspflow-scope-work-XXXXXX")"
  ( cd "$scopework" && git init -q )
  scopesession="waspflow-scope-$$"
  export WASPFLOW_LIB="$scopelib" WASPFLOW_HOME="$scopehome" WASPFLOW_TMUX_SESSION="$scopesession"

  scope_unit_of() { jq -r '.cgroup_scope // empty' "$scopehome/lanes/$1/state.json"; }
  scope_alive() {
    local unit; unit="$(scope_unit_of "$1")"
    [[ -n "$unit" ]] || return 1
    systemctl --user list-units --type=scope --all --no-legend 2>/dev/null | awk '{print $1}' | grep -qxF "$unit"
  }
  scope_cgroup_pids() {
    local unit cgpath; unit="$(scope_unit_of "$1")"
    [[ -n "$unit" ]] || return 0
    cgpath="$(systemctl --user show "$unit" -p ControlGroup --value 2>/dev/null)"
    [[ -n "$cgpath" ]] || return 0
    cat "/sys/fs/cgroup${cgpath}/cgroup.procs" 2>/dev/null || true
  }
  spawn_scope_lane() {
    local lane="$1" cmd="$2"
    ( cd "$scopework" && "$root/bin/waspflow" spawn --provider scopep --lane "$lane" -- "$cmd" >/dev/null )
  }

  # --- Case 1: normal completion. Scope is captured on spawn and is gone (unit
  #     exited on its own) once the command finishes and reap runs.
  spawn_scope_lane normal-done "sleep 0.3"
  [[ -n "$(scope_unit_of normal-done)" ]] || { echo "scope: spawn did not record cgroup_scope" >&2; exit 1; }
  for _ in $(seq 1 50); do scope_alive normal-done || break; sleep 0.1; done
  "$root/bin/waspflow" reap normal-done >/dev/null
  [[ "$(jq -r .status "$scopehome/lanes/normal-done/state.json")" == reaped ]] \
    || { echo "scope: normal completion did not reap" >&2; exit 1; }

  # --- Case 2: detached/setsid grandchild. tmux kill-window (alone) cannot
  #     reach it; reap's scope-kill must terminate it via the cgroup.
  spawn_scope_lane detached-grandchild \
    'setsid bash -c "sleep 300 & disown; wait" & disown; sleep 300'
  for _ in $(seq 1 50); do [[ -n "$(scope_cgroup_pids detached-grandchild)" ]] && break; sleep 0.1; done
  before="$(scope_cgroup_pids detached-grandchild)"
  [[ -n "$before" ]] || { echo "scope: detached grandchild never appeared in the cgroup" >&2; exit 1; }
  tmux kill-window -t "$scopesession:detached-grandchild" 2>/dev/null || true
  sleep 0.3
  after_window_kill="$(scope_cgroup_pids detached-grandchild)"
  [[ -n "$after_window_kill" ]] \
    || { echo "scope: test invalid — grandchild died from tmux kill-window alone" >&2; exit 1; }
  "$root/bin/waspflow" reap detached-grandchild >/dev/null
  for _ in $(seq 1 50); do [[ -z "$(scope_cgroup_pids detached-grandchild)" ]] && break; sleep 0.1; done
  [[ -z "$(scope_cgroup_pids detached-grandchild)" ]] \
    || { echo "scope: detached grandchild survived reap" >&2; exit 1; }
  for pid in $before; do
    kill -0 "$pid" 2>/dev/null && { echo "scope: grandchild pid $pid still alive after reap" >&2; exit 1; }
  done

  # --- Case 3: timeout path. reap must still terminate a scope even when the
  #     lane's main command is still running (long-lived / hung worker).
  spawn_scope_lane never-idle "sleep 300"
  never_pids="$(scope_cgroup_pids never-idle)"
  [[ -n "$never_pids" ]] || { echo "scope: never-idle lane produced no cgroup members" >&2; exit 1; }
  "$root/bin/waspflow" reap never-idle --force >/dev/null
  for pid in $never_pids; do
    kill -0 "$pid" 2>/dev/null && { echo "scope: timeout/force reap left a process running" >&2; exit 1; }
  done

  # --- Case 4: recovery pass (revise after a scope already died) must not
  #     error just because the OLD scope is gone; a fresh spawn/revise cycle
  #     records its own new scope independent of the exited one.
  spawn_scope_lane recovers "sleep 0.3"
  for _ in $(seq 1 50); do scope_alive recovers || break; sleep 0.1; done
  old_unit="$(scope_unit_of recovers)"
  "$root/bin/waspflow" revise recovers -- "true" >/dev/null 2>&1 || true
  "$root/bin/waspflow" reap recovers --force >/dev/null
  [[ "$(jq -r .status "$scopehome/lanes/recovers/state.json")" == reaped ]] \
    || { echo "scope: recovery-then-reap path failed" >&2; exit 1; }
  systemctl --user list-units --type=scope --all --no-legend 2>/dev/null | awk '{print $1}' | grep -qxF "$old_unit" \
    && { echo "scope: stale scope from before recovery was never cleaned up" >&2; exit 1; }

  # --- Case 5: active unrelated process must be untouched. A plain shell
  #     scope started OUTSIDE waspflow (no lane ever owns it) must survive
  #     reaping every waspflow-owned lane.
  bystander_unit="wf-verify-bystander-$$.scope"
  systemd-run --user --scope --unit="$bystander_unit" --collect --quiet -- sleep 300 >/dev/null 2>&1 &
  bystander_launcher=$!
  for _ in $(seq 1 50); do
    systemctl --user list-units --type=scope --all --no-legend 2>/dev/null | awk '{print $1}' | grep -qxF "$bystander_unit" && break
    sleep 0.1
  done
  spawn_scope_lane bystander-check "sleep 2"
  for _ in $(seq 1 50); do scope_alive bystander-check || break; sleep 0.1; done
  "$root/bin/waspflow" reap bystander-check >/dev/null
  systemctl --user list-units --type=scope --all --no-legend 2>/dev/null | awk '{print $1}' | grep -qxF "$bystander_unit" \
    || { echo "scope: reap killed an unrelated bystander scope" >&2; exit 1; }
  systemctl --user kill --kill-whom=all --signal=SIGKILL "$bystander_unit" >/dev/null 2>&1 || true
  systemctl --user stop "$bystander_unit" >/dev/null 2>&1 || true
  wait "$bystander_launcher" 2>/dev/null || true

  # --- Case 6: PID reuse protection. Forge a lane state.json whose recorded
  # unit name matches a CURRENTLY LIVE bystander scope but whose invocation id
  # does not (i.e. the name was reused / never actually this lane's own scope).
  # tmux_owned_lane_scope_target must refuse it — reap must not touch that
  # scope, proving ownership is checked by invocation id, not by name/PID text.
  reuse_unit="wf-verify-reuse-$$.scope"
  systemd-run --user --scope --unit="$reuse_unit" --collect --quiet -- sleep 300 >/dev/null 2>&1 &
  reuse_launcher=$!
  for _ in $(seq 1 50); do
    systemctl --user list-units --type=scope --all --no-legend 2>/dev/null | awk '{print $1}' | grep -qxF "$reuse_unit" && break
    sleep 0.1
  done
  spawn_scope_lane reuse-victim "sleep 2"
  for _ in $(seq 1 50); do scope_alive reuse-victim || break; sleep 0.1; done
  jq --arg u "$reuse_unit" '.cgroup_scope = $u | .cgroup_scope_invocation_id = "0000000000000000000000000000ffff"' \
    "$scopehome/lanes/reuse-victim/state.json" > "$scopehome/lanes/reuse-victim/state.next"
  mv "$scopehome/lanes/reuse-victim/state.next" "$scopehome/lanes/reuse-victim/state.json"
  "$root/bin/waspflow" reap reuse-victim >/dev/null
  systemctl --user list-units --type=scope --all --no-legend 2>/dev/null | awk '{print $1}' | grep -qxF "$reuse_unit" \
    || { echo "scope: PID-reuse protection failed — killed a scope with a mismatched invocation id" >&2; exit 1; }
  systemctl --user kill --kill-whom=all --signal=SIGKILL "$reuse_unit" >/dev/null 2>&1 || true
  systemctl --user stop "$reuse_unit" >/dev/null 2>&1 || true
  wait "$reuse_launcher" 2>/dev/null || true

  # --- Case 7: idempotent cleanup. Reaping an already-reaped lane (no scope,
  # no window) a second time must not error — tmux_kill_owned_lane_scope is a
  # no-op when there is nothing left to verify or kill.
  spawn_scope_lane idempotent "sleep 2"
  for _ in $(seq 1 50); do scope_alive idempotent || break; sleep 0.1; done
  "$root/bin/waspflow" reap idempotent >/dev/null
  ...
  echo "waspflow verify: skipping descendant-scope tests (no systemd --user available)" >&2
fi
