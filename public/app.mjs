...
export function providerCapacitySubject(identity) {
  ...
  const provider = account.provider || account.name || 'provider';
...
function receipt(task) {
  const fields = [
    ['Model', task.model || task.receipt?.model], ['Tokens', task.tokens || task.receipt?.tokens || tokenUsage(task.receipt)],
    ['Duration', task.duration || task.receipt?.duration], ['Sandbox', task.sandbox_id || task.receipt?.sandbox_id],
  ].filter(([, value]) => value);
...
function tokenUsage(receipt = {}) {
  const input = receipt.input_tokens ?? receipt.tokens_in;
  const output = receipt.output_tokens ?? receipt.tokens_out;
...
function activityView(ledger, requests, select) {
  ...
  return element('div', { className: 'view-stack' },
    panel('Contribution history', 'Every completed task leaves a private receipt.', contributionRows.length ? element('ul', { className: 'history-list' }, ...contributionRows.map((entry) => element('li', {}, element('div', {}, element('strong', { text: entry.display_id || entry.task_name || 'Federation task' }), element('p', { className: 'muted', text: formatDate(entry.finished_at || entry.settled_at) })), element('div', { className: 'history-meta', text: [entry.duration, entry.model, entry.tokens || tokenUsage(entry)].filter(Boolean).join(' · ') || 'Receipt pending' })))) : element('div', { className: 'empty-state' }, element('strong', { text: 'Your contribution history will appear here.' }), element('p', { text: 'When you complete a task, Waspflow records what ran and when.' }))),
    ...
    panel('Accounts in use', 'These are the identities Waspflow uses at rest.',
      element('dl', { className: 'identity-list' },
        ...
        ...accounts.flatMap((account) => [
          element('dt', { text: account.provider || account.name || 'Provider' }),
          element('dd', { text: [account.email, account.tier, account.authenticated === false ? 'needs sign-in' : 'signed in'].filter(Boolean).join(' · ') }),
